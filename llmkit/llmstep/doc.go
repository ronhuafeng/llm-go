...
// will occur. A final rejected attempt returns ErrExhausted without invoking
...
//
// Run publishes owned snapshots of attempt, judgment, and repair
// slices. Typed outputs inside those snapshots follow ordinary Go value
// semantics and are not generically deep-cloned.
//
// Run observes context cancellation before Render and after successful
...
// but cancellation is observed at its boundary, Run returns a StepError
// wrapping ctx.Err at that phase and retains completed output and evidence in
// the result. Cancellation after the final observation may race with
// a successful return.
//
