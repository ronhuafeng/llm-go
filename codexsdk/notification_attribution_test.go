...
func TestAttributionExtractsGeneratedIdentity(t *testing.T) {
	tests := []struct {
		...
		{"auth recovery turn fields", `{"method":"modelProvider/authRecoveryStarted","params":{"threadId":"thread-a","turnId":"turn-a","provider":"openai","message":"recovering"}}`, notificationAttributionTurn, "thread-a", "turn-a"},
