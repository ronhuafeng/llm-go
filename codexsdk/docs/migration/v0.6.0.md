...
The verified replacement resolves through the public Go proxy as
...
The main package is intentionally flattened to the new module root. There is
no `codexsdk/codexsdk` compatibility package or root re-export; consumers must
apply the complete import mapping above.

The new repository is the sole active source. The legacy path receives no
feature or security updates after `v0.5.1`, although its immutable tags remain
proxy-resolvable.
